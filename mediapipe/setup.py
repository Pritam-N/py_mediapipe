...
SUBDIR_INIT_PY_FILES = [
    ...
    os.path.join(MP_ROOT_PATH, 'mediapipe/graphs/hair_segmentation/__init__.py'),
]
...
class BuildBinaryGraphs(build.build):
  ...
  def run(self):
    ...
    bazel_command = [
      ...
      'mediapipe/graphs/hair_segmentation/hair_segmentation_cpu_binary_graph',
    ]
    if not self.link_opencv and not IS_WINDOWS:
      bazel_command.append('--define=OPENCV=source')
      if subprocess.call(bazel_command) != 0:
        sys.exit(-1)
      output_name = 'hair_segmentation/hair_segmentation_cpu.binarypb'
